...
public class LobbyBoard {
    ...
    public void createScoreboard(Player player) {
        ...
        objective.setDisplayName("§ePokemon§3Battle §7| " + "§3" + Utils.getDurationString(Pokebattle.getInstance().getLobbyCountdown().getTime()));
    ...
    public void updateTitle() {
        boards.keySet().forEach(scoreboard -> {
            scoreboard.getObjective(DisplaySlot.SIDEBAR).setDisplayName("§ePokemon§3Battle §7| " + "§3" +
                    Utils.getDurationString(Pokebattle.getInstance().getLobbyCountdown().getTime())
            );
        });
    }
